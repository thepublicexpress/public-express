...
<head>
    ...
    <title>@yield('title', 'द पब्लिक एक्सप्रेस')</title>
...
<body>
    ...
    <div class="top-bar">
        <div class="container d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center">
                ...
                <span class="text-white fw-semibold"><i class="far fa-calendar-alt me-1"></i> {{ date('d M Y, l') }}</span>
        ...
        <div class="container">
            <div class="row align-items-center g-3">
                <div class="col-12 col-md-3 d-flex align-items-center justify-content-between">
                    ...
                    <a href="{{ url('/') }}" class="mx-auto mx-md-0">
                        <img src="{{ asset('images/logo.png') }}" class="logo-img" alt="द पब्लिक एक्सप्रेस">
                    </a>
                    <div style="width: 30px;" class="mobile-menu-hamburger"></div>
                </div>
                
                <div class="col-12 col-md-6 d-flex justify-content-center">
                    <form action="{{ route('news.search') }}" method="GET" class="search-container">
    ...
    <nav class="nav-bar desktop-nav">
        <div class="container p-0">
            <div class="d-flex align-items-center flex-wrap" style="overflow-x: auto; white-space: nowrap; -webkit-overflow-scrolling: touch;">
                ...
                <!-- होम -->
                <a href="{{ url('/') }}" class="nav-link-custom home-btn {{ request()->is('/') ? 'active' : '' }}">
                ...
                @foreach($categories as $cat)
                    <a href="{{ route('category.show', $cat->slug) }}" class="nav-link-custom {{ request()->is('category/'.$cat->slug) ? 'active' : '' }}">
                    ...
                    <ul class="dropdown-menu">
                        @foreach($states as $state)
                            @if($state->districts->count() > 0)
                                <li><a class="dropdown-item fw-bold" href="{{ route('news.state', $state->slug) }}">
                                    <i class="fas fa-flag me-1"></i> {{ $state->display_name }}
                                </a></li>
                                @foreach($state->districts as $district)
                                    <li><a class="dropdown-item" href="{{ route('news.district', $district->slug) }}">
                                        <i class="fas fa-location-dot me-1"></i> {{ $district->display_name }}
                                    </a></li>
                                @endforeach
                            @endif
                        @endforeach
                ...
                <!-- 🏆 लीडरबोर्ड -->
                <a href="{{ route('leaderboard') }}" class="nav-link-custom text-warning {{ request()->routeIs('leaderboard') ? 'active' : '' }}">
                    🏆 लीडरबोर्ड
                </a>
    ...
    <div class="collapse bg-dark" id="mobileNavCollapse">
        <div class="p-3 d-flex flex-column gap-2">
            <a href="{{ url('/') }}" class="text-white text-decoration-none py-2 border-bottom border-secondary fw-bold">🏠 होम</a>
            
            @foreach($categories as $cat)
                <a href="{{ route('category.show', $cat->slug) }}" class="text-white text-decoration-none py-2 border-bottom border-secondary fw-bold">
                    {{ $cat->icon ?? '📰' }} {{ $cat->display_name }}
                </a>
            @endforeach
            
            <a href="{{ route('login') }}" class="text-white text-decoration-none py-2 fw-bold text-danger">🔐 लॉगिन</a>
            <a href="{{ route('otp.login') }}" class="text-white text-decoration-none py-2 fw-bold text-success">📱 OTP लॉगिन</a>
        </div>
    </div>
    ...
    <div class="mobile-bottom-nav">
        <a href="{{ url('/') }}" class="mobile-nav-item active"><i class="fas fa-home"></i><span>होम</span></a>
        <a href="{{ route('category.show', 'national') }}" class="mobile-nav-item"><i class="fas fa-globe"></i><span>देश</span></a>
        <a href="{{ route('category.show', 'politics') }}" class="mobile-nav-item"><i class="fas fa-balance-scale"></i><span>राजनीति</span></a>
        <a href="{{ route('category.show', 'education') }}" class="mobile-nav-item"><i class="fas fa-graduation-cap"></i><span>शिक्षा</span></a>
    </div>

    <!-- FOOTER -->
    <footer class="bg-dark text-white text-center py-4 mt-5 d-none d-md-block">
        &copy; {{ date('Y') }} द पब्लिक एक्सप्रेस. सर्वाधिकार सुरक्षित।
    </footer>
