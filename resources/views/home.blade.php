...
<body class="antialiased">
    ...
    <div id="mobileMenuPanel" class="mobile-menu-panel flex flex-col justify-between">
        <div>
            <div class="p-4 bg-slate-900 flex items-center justify-between text-white border-b-4 border-brand">
                <a href="/" class="flex flex-col">
                    <img src="{{ asset('images/logo.png') }}" alt="" class="h-10 w-auto object-contain" onerror="this.style.display='none'; document.getElementById('mob-text-logo').style.display='block';">
        ...
        <div class="p-4 border-t border-slate-100 bg-slate-50 space-y-2">
            <a href="{{ route('leaderboard') }}" class="flex justify-center items-center gap-2 p-3 w-full bg-slate-900 text-white font-bold rounded-xl shadow-sm text-sm">🏆 सर्वश्रेष्ठ रिपोर्टर सूची</a>
            <a href="{{ route('login') }}" class="flex justify-center items-center gap-2 p-3 w-full bg-brand text-white font-bold rounded-xl shadow-sm text-sm">🔐 लॉगिन</a>
            <a href="{{ route('otp.login') }}" class="flex justify-center items-center gap-2 p-3 w-full bg-green-600 text-white font-bold rounded-xl shadow-sm text-sm">📱 OTP लॉगिन</a>
        </div>
    </div>
    ...
    <header class="bg-white shadow-md sticky top-0 z-50 border-b-4 border-brand">
        <div class="container mx-auto px-4 md:px-6 py-2 flex items-center justify-between gap-4 relative">
            ...
            <div class="absolute inset-x-0 mx-auto flex justify-center md:static md:inset-auto md:mx-0 md:justify-start pointer-events-none md:pointer-events-auto">
                <a href="/" class="flex items-center flex-shrink-0 group py-1 min-w-[160px] md:min-w-[200px] justify-center md:justify-start pointer-events-auto">
                    <img src="{{ asset('images/logo.png') }}" alt="" class="h-11 md:h-16 w-auto object-contain transform group-hover:scale-102 transition duration-300" onerror="this.style.display='none'; document.getElementById('desk-text-logo').style.display='flex';">
            ...
            <div class="flex gap-3 flex-shrink-0 items-center z-10">
                <a href="{{ route('login') }}" class="bg-brand text-white px-5 py-2.5 rounded-none text-xs font-black uppercase tracking-wider hover:bg-slate-950 transition shadow-md items-center gap-2 hidden md:flex">
                    <span>🔐 लॉगिन</span>
                </a>
            </div>
        </div>
        ...
        <nav class="bg-slate-900 hidden md:block shadow-inner border-t border-slate-800">
            <div class="container mx-auto px-6 py-0.5 flex items-center gap-1 text-sm font-black text-white whitespace-nowrap overflow-x-visible relative">
                ...
                @foreach($categories as $cat)
                    <a href="{{ route('category.show', $cat->slug) }}" class="px-4 py-3 hover:bg-brand hover:text-white text-slate-200 text-xs tracking-wider transition font-bold">
                        {{ $cat->icon ?? '📰' }} {{ $cat->display_name }}
                    </a>
                @endforeach
                        ...
                        @foreach($states ?? [] as $state)
                            @if($state->districts->count() > 0)
                                <div class="relative group/district border-b border-slate-800/60">
                                    <a href="{{ route('news.state', $state->slug) }}" class="w-full text-left px-4 py-3 hover:bg-brand transition-colors flex justify-between items-center text-xs text-slate-200 font-bold">
                                        📍 {{ $state->display_name }} <i class="fas fa-chevron-right text-[9px] opacity-60"></i>
                                    </a>
                                    <div class="absolute left-full top-0 w-44 bg-slate-900 shadow-xl opacity-0 invisible group-hover/district:opacity-100 group-hover/district:visible transition-all duration-150 z-50 border-l border-slate-800">
                                        @foreach($state->districts as $district)
                                            <a href="{{ route('news.district', $district->slug) }}" class="block px-4 py-2.5 hover:bg-brand text-xs text-slate-300 font-semibold border-b border-slate-800/40">
                ...
                <div class="relative rounded-none overflow-hidden shadow-2xl group bg-slate-900 h-[360px] md:h-[460px] border-b-4 border-brand">
                    ...
                    <div class="absolute inset-0 bg-gradient-to-t from-black via-black/70 to-transparent p-5 md:p-8 flex flex-col justify-end">
                        <div class="flex gap-2 mb-3">
                            ...
                            <span class="bg-white text-slate-950 text-[10px] font-black px-3 py-1">{{ $latestNews->first()->category->display_name ?? 'ताजा खबर' }}</span>
                        </div>
                        <h2 class="text-xl md:text-3xl font-black text-white mb-3 leading-tight tracking-tight">
                            <a href="{{ route('news.show', $latestNews->first()->slug) }}" class="hover:text-brand transition">{{ $latestNews->first()->title }}</a>
                        </h2>
            ...
            <div class="flex flex-col justify-between gap-6">
                ...
                <div class="bg-white rounded-none shadow-md p-5 border-t-4 border-slate-950 flex-1">
                    ...
                    <div class="grid grid-cols-2 gap-3.5">
                        <div class="bg-slate-50 p-3.5 border-l-4 border-brand">
                            <p class="text-2xl font-black text-slate-950">{{ $totalNews ?? $latestNews->total() ?? 0 }}</p>
                        ...
                        <div class="bg-slate-50 p-3.5 border-l-4 border-slate-900">
                            <p class="text-2xl font-black text-slate-900">{{ $todayNews ?? 0 }}</p>
                ...
                <div class="bg-white rounded-none shadow-md p-5 border-t-4 border-brand flex-1">
                    <div class="flex justify-between items-center mb-4 border-b border-slate-100 pb-2">
                        ...
                        <a href="{{ route('leaderboard') }}" class="text-brand text-xs font-black hover:underline">सूची देखें →</a>
                    </div>
                    <div class="space-y-3">
                        ...
                        <div class="flex items-center gap-3 p-1.5 hover:bg-slate-50 transition border-b border-slate-50 last:border-0">
                            <div class="w-5 text-center font-black text-slate-900 text-xs">#{{ $index + 1 }}</div>
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-xs font-black text-slate-900 truncate">{{ $reporter->name }}</p>
                                <p class="text-[10px] font-bold text-slate-400 mt-0.5">{{ number_format($reporter->points ?? 0) }} पॉइंट</p>
                            </div>
                        </div>
                        @endforeach
                    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white rounded-none shadow-md border border-slate-200 overflow-hidden transform hover:-translate-y-1 hover:shadow-lg transition flex flex-col justify-between">
                    <div>
                        <div class="relative overflow-hidden aspect-video bg-slate-100 border-b-2 border-slate-100">
                            <img src="{{ url('/serve-image/' . urlencode($item->featured_image)) }}" 
                                 ...
                                 loading="lazy"
                                 onerror="this.src='{{ asset('images/default-news.jpg') }}'">
                            <span class="absolute top-3 left-3 bg-brand text-white text-[9px] font-black tracking-wider px-2 py-0.5 shadow-md">{{ $item->category->display_name ?? 'ताजा समाचार' }}</span>
                        </div>
                        <div class="p-4">
                    ...
                    <div class="px-4 pb-4 pt-3 flex justify-between items-center border-t border-slate-100">
                        ...
                        <a href="{{ route('news.show', $item->slug) }}" class="text-brand text-xs font-black hover:underline flex items-center gap-0.5 tracking-wider">पूरी खबर →</a>
                    </div>
                </div>
                @endforeach
            </div>
    ...
    <footer class="bg-slate-950 text-slate-400 pt-12 pb-24 md:pb-8 border-t-4 border-brand">
        <div class="container mx-auto px-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8 mb-8">
                <div>
                    <img src="{{ asset('images/logo.png') }}" alt="द पब्लिक एक्सप्रेस" class="h-12 w-auto object-contain mb-3 brightness-95" onerror="this.style.display='none';">
                ...
                <div>
                    ...
                    <ul class="space-y-2 text-xs font-bold">
                        ...
                        <li><a href="{{ route('leaderboard') }}" class="hover:text-white transition">🏆 रिपोर्टर लीडरबोर्ड</a></li>
                    </ul>
                </div>
                <div>
                    <h4 class="font-black text-xs text-white tracking-widest mb-3 border-b border-slate-900 pb-1">अकाउंट</h4>
                    <ul class="space-y-2 text-xs font-bold">
                        <li><a href="{{ route('login') }}" class="hover:text-white transition">🔐 लॉगिन</a></li>
                        <li><a href="{{ route('register') }}" class="hover:text-white transition">📝 रजिस्टर</a></li>
                        <li><a href="{{ route('otp.login') }}" class="hover:text-white transition">📱 OTP लॉगिन</a></li>
                    </ul>
                </div>
                <div>
            ...
            <div class="border-t border-slate-900 pt-6 text-center text-xs font-black text-slate-600 tracking-wider">
                © {{ date('Y') }} द पब्लिक एक्सप्रेस नेटवर्क. ऑल राइट्स रिजर्व्ड।
            </div>
        </div>
    </footer>
    ...
    <nav class="md:hidden fixed bottom-4 left-4 right-4 h-16 glass-nav shadow-2xl border-2 border-slate-950/10 z-[9999] flex justify-around items-center px-2 rounded-xl">
        ...
        @foreach($categories as $cat)
            @if($cat->slug == 'national')
                <a href="{{ route('category.show', $cat->slug) }}" class="flex flex-col items-center justify-center w-12 h-12 text-slate-950 hover:text-brand rounded-xl">
